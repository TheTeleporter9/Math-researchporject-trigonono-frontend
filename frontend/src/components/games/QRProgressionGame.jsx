...
import { researchStorage, showSuccess, showError, sessionsStorage } from '../../utils/storage'
...
export default function QRProgressionGame({ session, studentData }) {
  ...
  const loadCurrentLevel = async () => {
    try {
      const progress = await sessionsStorage.getStudentProgress(session.id, studentData.anonymousId)
  ...
  const initializeLevel = async () => {
    ...
    try {
      ...
      const levelData = `LEVEL_${currentLevel}_${session.id}_${studentData.anonymousId}_${seed}`
  ...
  const handleAnswer = (questionId, answer) => {
    ...
    if (qrData) {
      ...
      // Check if QR is complete
      if (isQRComplete(qrData, newFilledCells)) {
        setWaitingForApproval(true)
        // Start polling for level advancement
        const pollInterval = setInterval(async () => {
          try {
            const progress = await sessionsStorage.getStudentProgress(session.id, studentData.anonymousId)
            if (progress.currentLevel > currentLevel) {
    ...
    <div>
      ...
      <div className="grid grid-2">
        ...
        <div className="panel">
          ...
          {waitingForApproval ? (
            <p className="text-muted">
              Zeige diesen QR-Code deinem Lehrer, um zur nächsten Stufe zu gelangen.
            </p>
          ) : (
            <p className="text-muted">
              Fülle Zellen, indem du Fragen richtig beantwortest.
            </p>
          )}
          ...
          <div className="text-center mt-2">
            ...
            {waitingForApproval && (
              <p className="text-success">Warte auf Genehmigung des Lehrers...</p>
            )}
          </div>
        </div>
      </div>
