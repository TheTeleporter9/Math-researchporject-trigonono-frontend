...
export default function TriangleMatchingGame({ session, studentData }) {
  ...
  const createDisplayPlan = (seed, candidateCount) => {
    ...
    const referenceKeys = keys.slice(0, 3)
    const missingKeys = keys.slice(3)

    // Shuffle missing keys too (so "spread about" varies)
    for (let i = missingKeys.length - 1; i > 0; i--) {
      const j = Math.floor(rng() * (i + 1))
      const tmp = missingKeys[i]
      missingKeys[i] = missingKeys[j]
      missingKeys[j] = tmp
    }

    const toIndices = (k) => {
      if (k.t === 's') return { side: k.i, angle: null }
      return { side: null, angle: k.i }
    }

    const refSides = referenceKeys.filter(k => k.t === 's').map(k => k.i).sort()
    const refAngles = referenceKeys.filter(k => k.t === 'a').map(k => k.i).sort()

    const candidates = Array.from({ length: candidateCount }, (_, idx) => {
      const extra = missingKeys[idx % missingKeys.length]
      const show = [...referenceKeys, extra]
      return {
        sideIndices: show.filter(k => k.t === 's').map(k => k.i).sort(),
        angleIndices: show.filter(k => k.t === 'a').map(k => k.i).sort(),
        extra
      }
    })

    return {
      reference: { sideIndices: refSides, angleIndices: refAngles },
      candidates,
      missingKeys: missingKeys.map(toIndices)
    }
  }
  ...
  useEffect(() => {
    ...
    const baseSeed = Number.isFinite(session?.config?.seed_base) ? session.config.seed_base : generateSeed()
    const seed = baseSeed + currentLevel * 1000 // Deterministic per session + level
    const config = {
      baseSize: 100,
    ...
    try {
      ...
      setDisplayPlan(createDisplayPlan(seed, set.candidates.length))
      if (import.meta.env.DEV) {
        const correctIndex = set.candidates.findIndex(t => t.isMatch)
        // Note: in Vite this shows in the browser console; for terminal output use scripts/debug-game1.mjs
        console.log(`[Game1 debug] level=${currentLevel} seed=${seed} correctIndex=${correctIndex}`, {
          targetSides: set.targetSides?.map(s => Number(s.toFixed(2))),
          targetAngles: set.targetAngles?.map(a => Number(a.toFixed(2))),
          targetMeasured: measureTriangle(set.target)
        })
      }
      setTriangleSet(set)
  ...
  const refPlan = displayPlan?.reference
        ...
        <div className="mb-4">
          ...
          <div style={{ background: '#f8f9fa', padding: '1rem', borderRadius: '8px', border: '2px solid #dee2e6' }}>
            <TriangleDisplay
              ...
              size={300}
              label="Referenz"
              showSideIndices={refPlan?.sideIndices}
              showAngleIndices={refPlan?.angleIndices}
              onMeasure={(meas) => {
            ...
            {candidates.map((triangle, index) => (
              ...
              >
                <TriangleDisplay
                  ...
                  showSideIndices={displayPlan?.candidates?.[index]?.sideIndices}
                  showAngleIndices={displayPlan?.candidates?.[index]?.angleIndices}
                  onMeasure={(meas) => {
...
function TriangleDisplay({ triangle, size, label, onMeasure, showSideIndices, showAngleIndices }) {
  ...
  const showSidesSet = Array.isArray(showSideIndices) ? new Set(showSideIndices) : null
  const showAnglesSet = Array.isArray(showAngleIndices) ? new Set(showAngleIndices) : null
  const showSide = (i) => !showSidesSet || showSidesSet.has(i)
  const showAngle = (i) => !showAnglesSet || showAnglesSet.has(i)
  ...
  return (
    <div>
      ...
      <div className="text-center mt-2" style={{ fontSize: '0.875rem', color: '#6c757d' }}>
        <span>
          Seiten: {measurements.sides.filter((_, i) => showSide(i)).map(s => s.toFixed(1)).join(', ') || '—'}
        </span>
        <span> | </span>
        <span>
          Winkel: {measurements.angles.filter((_, i) => showAngle(i)).map(a => a.toFixed(0)).join('°, ') || '—'}°
        </span>
      </div>
    </div>
  )
